...
## Discussion
The main objective of this climate impact analysis was to determine if rising temperature is correlated with increased wildfire frequency and size. This study was successful in concluding that in the southern central regions of British Columbia, from May to September 2021, high temperatures were a primary cause of wildfire occurrence resulting in more fires clustered together where temperatures were greater. Elsewhere in BC, particularly in the northern regions, temperature is not seen to influence wildfire occurrence as strongly and when looking at Figure 7, the kernel density estimation we can see there is significantly less clustering at we move north. With this relationship in mind, the month of July experienced the largest area burned by fire followed by June. It is during these months that BC typically experiences maximum yearly temperatures. During this time the majority of fires were approximately 0.09 ha in size.  

Organizations such as BC Wildfire Service could utilize the results from this tutorial to determine future spatial trends in wildfires based on temperature highs and mitigate the potential damage, protecting wildlife habitats and residential communities. 

A few major limitations to this study include data availability and quality as well as GWR's linear relationship assumption. Since both datasets acquired for temperature and fire contained many areas without observations there were gaps in the analyses that led to errors such as over or under-prediction of the interpolated surface models. Since the results of these models were used in the GWR, these outputs are also littered with error/inaccuracy. The fire dataset also represents bias as typically larger more destructive fires are prioritized and underreport smaller fires. In terms of the GWR model, a linear relationship may discredit complex interactions between temperature and fire variables and not be sufficient for modeling purposes. For this reason, I would recommend integrating other climate variables such as wind, precipitation, or humidity to better understand fire patterns in northern BC. Given the GWR results, it is also worth noting that this tutorial did not do an efficient job of correlating temperature and fire in northern regions. Future analysis with more complete data should be conducted in this area to better grasp how temperature effects fires on a global scale of BC.
